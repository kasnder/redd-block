...
### App lifecycle
Closing the window **hides to tray** and keeps all watchers running. **There is no quit gesture at all** — the tray icon has no menu by design (left-click reveals the window, right-click does nothing), and both `RunEvent::ExitRequested` and, on macOS, an `applicationShouldTerminate:` hook intercept every other exit route and turn it into a hide-window. The `ALLOW_EXIT` flag those guards check is never set to `true`; it exists as the seam an authorised exit would use if one were ever added. The only real way out is **in-app uninstall** (macOS) or the OS uninstaller (Windows), and `commands/uninstall.rs` deliberately calls `std::process::exit(0)` to bypass the guards rather than going through `ALLOW_EXIT`. Enforcement continues across window close. An EULA gate (revision-based, `CURRENT_EULA_REVISION` in `src/app.js`) blocks post-acceptance startup hooks. v1.x cleanup (hosts strip, legacy daemon removal, may prompt for admin once) runs once via `src-tauri/src/commands/migration.rs`.
