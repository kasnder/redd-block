# Rum Architecture Reference (macOS, Windows, iOS)

Technical source-of-truth for how Rum works today and how earlier
...
- Windows: `%PROGRAMDATA%\Rum\redd-block-data.json` (legacy fallback: `%PROGRAMDATA%\ReDD Block\redd-block-data.json`)
...
`%PROGRAMDATA%\Rum\helper-state.json` (Windows, plus legacy `%PROGRAMDATA%\ReDD Block\helper-state.json`) until migration removes
...
Protected apps (Rum itself, Finder, shell processes) are never targeted.
...
|---|---|---|---|
| App data (canonical) | `/var/lib/redd-block/redd-block-data.json` | `%PROGRAMDATA%\Rum\redd-block-data.json` (legacy fallback: `%PROGRAMDATA%\ReDD Block\redd-block-data.json`) | App sandbox |
