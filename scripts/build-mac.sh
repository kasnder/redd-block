...
# `for-distribution/Rum.app`. For a shippable installer, run
...
echo "Building Rum for macOS (${BUILD_TARGET})..."
...
APP_SOURCE="${TARGET_DIR}/macos/Rum.app"

mkdir -p for-distribution

if [ -d "$APP_SOURCE" ]; then
  rm -rf "for-distribution/Rum.app"
  cp -R "$APP_SOURCE" "for-distribution/Rum.app"
fi

echo ""
echo "Build complete."
if [ -d "for-distribution/Rum.app" ]; then
  echo "  App: for-distribution/Rum.app"
fi
