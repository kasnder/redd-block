...
Write-Host "=== Building MSIX Package (ReDD Block) ===" -ForegroundColor Cyan
Write-Host ""
...
if (-not $IdentityName -or -not $Publisher) {
    ...
    Write-Host "  Find these under Partner Center -> Apps and games -> ReDD Block -> Product identity" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "  App: ReDD Block v$AppVersion" -ForegroundColor White
