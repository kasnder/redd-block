...
fn legacy_artefacts_present() -> bool {
    ...
    // dir (/var/lib/redd-block on macOS, C:\ProgramData\Rum on
    ...
    #[cfg(target_os = "windows")]
    {
        Path::new(r"C:\ProgramData\Rum\helper-state.json").exists()
...
fn run_elevated_macos(staged: &Path, status_path: &Path) -> ElevatedOutcome {
    ...
    let script = format!(
        r#"do shell script "{}" with administrator privileges with prompt "Rum needs to clean up the old hosts-file entries and remove the legacy helper.""#,
        escaped
    );
