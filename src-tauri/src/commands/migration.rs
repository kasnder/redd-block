...
fn legacy_artefacts_present() -> bool {
    ...
    // dir (/var/lib/redd-block on macOS, C:\ProgramData\Fristed on
...
fn run_elevated_macos(staged: &Path, status_path: &Path) -> ElevatedOutcome {
    ...
    let script = format!(
        r#"do shell script "{}" with administrator privileges with prompt "Fristed needs to clean up the old hosts-file entries and remove the legacy helper.""#,
        escaped
    );
