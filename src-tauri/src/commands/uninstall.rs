...
//!      ("Rum would like to control Finder") — we warn the
...
pub fn uninstall_self_macos(
    ...
    //    rather than hardcoding `/Applications/Rum.app` so a
    ...
    match try_trash_via_nsfilemanager(&bundle) {
        ...
        Err(e) => {
            ...
            // "Rum would like to control Finder" Automation
...
///     (containing `Rum.app/Contents/MacOS/redd-block`), so
...
fn spawn_native_host_killer(bundle: &str) -> std::io::Result<()> {
    // Pull just the bundle name (e.g. "Rum.app") so the match
...
/// "/Applications/Rum.app"). Walks up from the executable
...
///      hasn't granted Rum "App Management" permission, but
