...
//     Trade-off: no auto-uninstall when Rum goes away (user
...
//   Rum release on macOS install — that release tried the
...
//   once per Rum install — re-installs after a deliberate user
...
fn install_chromium(browser: BrowserTarget) -> std::io::Result<()> {
    ...
    // nothing. Avoids the macOS Sonoma+ "Rum would like to
...
/// brief Rum release that tried the policy approach before we
...
///
/// At Rum install time we want a one-shot scrub: clear the
