...
/// Every Chromium-family extension ID Rum treats as ReDD Focus.
...
impl BrowserTarget {
    ...
    fn manifest_dir(self) -> Option<PathBuf> {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            Some(local.join("Rum").join("native-host"))
        }
...
fn manifest_body(browser: BrowserTarget, binary_path: &str) -> serde_json::Value {
    let mut obj = json!({
        "name": HOST_NAME,
        "description": "Rum native messaging host",
        "path": binary_path,
        "type": "stdio",
    });
    match browser {
...
/// macOS Sonoma+ fires the "Rum would like to access data from
...
#[cfg(target_os = "windows")]
fn install_one(browser: BrowserTarget, binary: &str) -> std::io::Result<()> {
    // 1. Write the manifest to %LOCALAPPDATA%\Rum\native-host\.
    let dir = browser.manifest_dir().ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::Other, "cannot resolve manifest dir")
    })?;
    std::fs::create_dir_all(&dir)?;
